...
3. 将 `penguin-server-fabric-1.0.5.jar` 放入服务器 `mods/` 目录
4. 启动服务器，生成配置文件后关闭
...
`/查在线` 命令支持三种显示模式（优先级从高到低）：

1. **Markdown + 图片模式**（`use-markdown = true` 且 `post-img = true`）：推荐方式，Markdown 卡片内嵌 MOTD 图片
2. **纯图片模式**（`use-markdown = false` 且 `post-img = true`）：发送文本 + MOTD 图片
...
构建产物位于 `build/libs/penguin-server-fabric-1.0.5.jar`
